...
export interface ServicingRequestData {
  serviceRef: string;
  requestType: 'servicing' | 'out_of_area_enquiry';
  postcode: string;
  outwardCode?: string;
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>New Servicing Request</title>
</head>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f8fafc;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f8fafc; padding: 40px 20px;">
    <tr>
      <td align="center">
        <table width="700" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          <tr>
            <td style="background-color: #059669; padding: 30px; text-align: center;">
              <h1 style="margin: 0; color: #ffffff; font-size: 24px; font-weight: 700;">New Servicing Request</h1>
              <p style="margin: 8px 0 0 0; color: #d1fae5; font-size: 14px;">${data.serviceRef}</p>
            </td>
          </tr>
          <tr>
            <td style="padding: 40px;">
              ...
              <div style="background-color: ${data.coverageStatus === 'in_area' ? '#d1fae5' : '#fef3c7'}; border-left: 4px solid ${data.coverageStatus === 'in_area' ? '#10b981' : '#f59e0b'}; padding: 16px; margin-bottom: 24px; border-radius: 4px;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                  <div>
                    <div style="color: ${data.coverageStatus === 'in_area' ? '#065f46' : '#92400e'}; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px;">Coverage Status</div>
                    <div style="color: ${data.coverageStatus === 'in_area' ? '#065f46' : '#92400e'}; font-size: 16px; font-weight: 600;">${data.coverageStatus === 'in_area' ? 'In Service Area' : 'Out of Area Enquiry'}</div>
                  </div>
                  ${data.outwardCode ? `
                  <div style="text-align: right;">
                    <div style="color: ${data.coverageStatus === 'in_area' ? '#065f46' : '#92400e'}; font-size: 12px; margin-bottom: 4px;">Outward Code</div>
              ...
              ${(data.fuelType || data.boilerType || data.boilerMake || data.boilerModel || data.lastServiced) ? `
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 32px;">
                ...
                <tr>
                  <td style="padding: 12px 0; border-bottom: 1px solid #e2e8f0;">
                    <span style="color: #64748b; font-size: 14px;">Last Serviced</span>
                  </td>
                  <td style="padding: 12px 0; border-bottom: 1px solid #e2e8f0; text-align: right;">
                    <span style="color: #1e293b; font-size: 14px; font-weight: 600;">${data.lastServiced}</span>
                  </td>
                </tr>
                ` : ''}
              </table>
